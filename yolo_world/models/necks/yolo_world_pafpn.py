...
class YOLOWorldDualPAFPN(YOLOWorldPAFPN):
    ...
    def __init__(self,
                 ...
                 embed_channels: List[int],
                 num_heads: List[int],
                 deploy_guide_channels: int = None,
                 deepen_factor: float = 1.0,
                 widen_factor: float = 1.0,
                 num_csp_blocks: int = 3,
                 freeze_all: bool = False,
        ...
        self.deploy_guide_channels = deploy_guide_channels
        if deploy_guide_channels is None:
        ...
        else:  # for rep yolo world v1, only for deployment
            text_enhancder.update(
                dict(
                    image_channels=[int(x * widen_factor) for x in out_channels],
                    text_channels=deploy_guide_channels,
                    num_feats=len(out_channels),
                ))
            self.text_embeddings = nn.Parameter(torch.zeros(1, guide_channels, deploy_guide_channels))
    ...
    def forward(self, img_feats: List[Tensor], txt_feats: Tensor) -> tuple:
        ...
        if self.deploy_guide_channels is None:
            txt_feats = self.text_enhancer(txt_feats, inner_outs)
        else:
            txt_feats = self.text_enhancer(self.text_embeddings, inner_outs)
