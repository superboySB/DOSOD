...
@MODELS.register_module()
class RepContrastiveHead(BaseModule):
    """Contrastive Head for YOLO-World
    compute the region-text scores according to the
    similarity between image and text features
    Args:
...
class BNContrastiveHead(BaseModule):
    ...
    def forward(self, x: Tensor, w: Tensor) -> Tensor:
        """Forward function of contrastive learning."""
        x = self.norm(x)
        w = F.normalize(w, dim=-1, p=2)  # in text encoder, w has been L2 normalized, so for v2, this is redundant
...
@MODELS.register_module()
class RepYOLOWorldHeadModuleV1(YOLOWorldHeadModule):
    """
    yolo world head module for v1
    """
    def __init__(self,
                 *args,
                 embed_dims: int,
