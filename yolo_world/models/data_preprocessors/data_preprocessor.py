...
class YOLOWDetDataPreprocessor(DetDataPreprocessor):
    ...
    def deploy(self, batch_input: Tensor) -> Tensor:
        """
        efficient preprocessing for deployment
        the input is batch tensor of shape (N, C, H, W)
        the main step is to put normalization into forward
        """
        if batch_input.dtype != torch.float32:
            batch_input = batch_input.to(torch.float32)

        if self._enable_normalize:
            batch_input = (batch_input - self.mean) / self.std
