...
def main():
    ...
    if backend in (MMYOLOBackend.ONNXRUNTIME, MMYOLOBackend.OPENVINO,
        ...
        if not args.model_only:
            if args.without_bbox_decoder:
                assert args.without_nms is True, 'without_nms must be true if without_bbox_decoder is true'
                print_log('Export ONNX without bbox decoder and without NMS ...')
            else:
                if args.without_nms:
                    print_log('Export ONNX with bbox decoder and without NMS ...')
                else:
    ...
    if args.model_only:  # in most cases, modal_only is used for forward latency test only
